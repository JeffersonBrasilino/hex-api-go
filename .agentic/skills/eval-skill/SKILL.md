...
```bash
node .agentic/skills/eval-skill/scripts/scaffold-eval-workspace.cjs {skill_path} \
  [--ids {ids}]
```
...
```bash
node .agentic/skills/eval-skill/scripts/write-timing.cjs \
...
```bash
node .agentic/skills/eval-skill/scripts/finalize-grading.cjs {workspace}/iteration-{N}
```
...
```bash
node .agentic/skills/eval-skill/scripts/compute-benchmark.cjs {workspace}/iteration-{N}
```
