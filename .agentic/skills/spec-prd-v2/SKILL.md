...
- **Offer a recommended answer.** The developer is not a PM — for each question, propose a sensible
  default they can simply confirm (e.g. `Sugiro X. Confirma ou prefere outro caminho?`). This lowers
  their burden and speeds convergence.
...
- **Personal data → LGPD.** When CPF, email, birth date, address, etc. appear, explore each of
  the three LGPD dimensions **one per turn** — never bundle them in a single message:
...
- **Gate reminder during criteria iteration.** If the user proposes changes to acceptance criteria
  mid-interview, incorporate them — but remind that **Step 2 approval is still mandatory** before
  `PRD.md` is generated, even when criteria are already drafted.
...
3. Load `references/prd-template.md` and write `PRD.md` following it exactly, in pt-BR.
   - Every non-functional requirement **must** use the `RNF-0X` identifier (e.g. `**RNF-01 (Performance):**`).
     Plain-text RNFs without this identifier are not acceptable.
