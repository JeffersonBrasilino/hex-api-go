...
//
// Returns: a map with accessToken and refreshToken on success, or an error — domain.UserBlockedError
// when the user is locked out, ddgo.ValidationError when the credentials are invalid, or any error
// surfaced by the underlying contracts.
//
// Hybrid token: the real refresh token generated by tokenGenerator never leaves the server — it is
// persisted server-side, keyed by sessionId, via loginSessionStore. The client only ever receives
// sessionId, exposed under the "refreshToken" key, as an opaque handle to that server-side record.
