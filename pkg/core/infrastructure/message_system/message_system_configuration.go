...
func registerDefaultCommandBus(container container.Container[any, any]) {
	channel := channel.NewPointToPointChannel(defaultCommandChannelName)
	channel.Subscribe(func(msg *message.Message) {
		router.NewRecipientListRouter(container).Handle(msg)
	})
	container.Set(defaultCommandChannelName, channel)
...
func registerDefaultQueryBus(container container.Container[any, any]) {
	channel := channel.NewPointToPointChannel(defaultQueryChannelName)
	channel.Subscribe(func(msg *message.Message) {
		router.NewRecipientListRouter(container).Handle(msg)
	})
	container.Set(defaultQueryChannelName, channel)
	AddGateway(endpoint.NewGatewayBuilder(
...
func AddActionHandler[
	T endpoint.Action,
	U any,
](handler endpoint.ActionHandler[T, U]) {
	action := *new(T)
	...
	outboundChannelBuilders.Set(
		action.Name(),
		endpoint.NewActionHandleActivatorBuilder(
			action.Name(),
			handler,
		),
	)
...
func Start() {
	registerDefaultCommandBus(messageSystemContainer)
	registerDefaultQueryBus(messageSystemContainer)
