...
func TestAddHandler(t *testing.T) {
	t.Parallel()
	t.Run("should add a handler to the router", func(t *testing.T) {
		r := NewRouter()
		h := &dummyHandler{}
		r.AddHandler(h)
		if len(r.handlers) != 1 {
...
func TestHandle_AllSuccess(t *testing.T) {
	t.Parallel()
	t.Run("should handle all messages successfully", func(t *testing.T) {
		r := NewRouter()
		h1 := &dummyHandler{}
		h2 := &dummyHandler{}
...
func TestHandle_HandlerReturnsNil(t *testing.T) {
	t.Parallel()
	t.Run("should return nil when a handler returns nil", func(t *testing.T) {
		r := NewRouter()
...
func TestHandle_HandlerReturnsError(t *testing.T) {
	t.Parallel()
	t.Run("should return error when a handler returns error", func(t *testing.T) {
		r := NewRouter()
